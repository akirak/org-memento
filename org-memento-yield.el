...
(defun org-memento-yield--activities-1 (rule taxy)
  ...
  (let* ((plist (org-memento-yield-backtrack-spec rule))
         ...
         (done-keywords (with-current-buffer (org-memento--buffer)
                          org-done-keywords)))
    (cl-labels
        ((match-group (group)
           (equal (seq-take group (length group-path))
                  group-path))
         (map-record (record)
           (let* ((extra-fields (seq-drop record 5))
                  (todo (plist-get extra-fields :todo-keyword)))
             (when (and (match-group (plist-get extra-fields :group))
                        (or (equal todo org-memento-todo-keyword-for-success)
                            ;; Ignore failed activities
                            (not (member todo done-keywords))))
               record)))
