...
(cl-defmethod org-memento-duration ((x org-memento-block))
  (or (when-let* ((ts (org-memento-block-active-ts x))
        ...
        (unless (time-equal-p start-time end-time)
          (/ (- (float-time end-time) (float-time start-time))
             60)))
      (when-let (effort (org-element-property :EFFORT (org-memento-headline-element x)))
        (org-duration-to-minutes effort))))

(cl-defmethod org-memento-ending-time ((x org-memento-block))
  (or (when-let (started (org-memento-started-time x))
        (or (when-let* ((ts (org-memento-block-active-ts x))
                        (end-time (org-timestamp-to-time ts 'end))
                        (start-time (org-timestamp-to-time ts))
                        (duration-sec (unless (time-equal-p start-time end-time)
                                        (- end-time start-time))))
              (+ started duration-sec))
            (when-let (effort (org-element-property
                               :EFFORT (org-memento-headline-element x)))
              (+ started (* 60 (org-duration-to-minutes effort))))))
      (when-let (ts (org-memento-block-active-ts x))
        (let* ((start-time (when ts (org-timestamp-to-time ts)))
               (end-time (when ts (org-timestamp-to-time ts 'end))))
          (or (unless (time-equal-p end-time start-time)
                (float-time end-time))
              (when-let* ((effort (org-element-property :EFFORT (org-memento-headline-element x)))
                          (duration (org-duration-to-minutes effort)))
                (+ (float-time start-time) (* 60 duration))))))))
...
(cl-defun org-memento-def-zone (title &key description duration complete groups
                                      block-p planning-item-p order-p group-p
                                      duration-p
                                      children)
  (declare (indent 1))
  (let ((children (delq nil children)))
    (cl-flet*
        ((predicate (x)
           (or (and duration-p
                    (when-let (duration
         ...
         (predicate2 (x)
           (or (predicate x)
               (seq-some (apply-partially #'f x)
                         (mapcar #'taxy-predicate children)))))
